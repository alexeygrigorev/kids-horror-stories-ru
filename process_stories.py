...
def create_story(base64_image):
    ...
    Для названий не используй слова как "проклятый", "проклятье", "мрачный", "тайна". Не используй никакое форматирование
...
def edit_story(story):
    prompt = f"""
    Ты опытный редактор историй ужасов. Исправь эту историю.
    Сделай так, чтобы вся грамматика была понятна и корректна.
